...
## Keuze rasterlaag
Voor het meetnet kiezen we een GRTS-laag van Vlaanderen met level 8, t.t.z. met cellen van 8192 m (figuur \@ref(fig:meetnetontwerp)).
...
Voor deze selectie werd met een zoekstraal van 3 meter rond een meetpunt gewerkt. 
We selecteren enkel de meetpunten die grondwaterpeilen meten (piëzometer of peilbuis). Voor de eenvoud benoemen we ze peilbuizen. 
...
Om dat te vermijden, voegen we de verschillende habitattypen samenvoegen die tot eenzelfde GT-groep behoren.
...
kable(tubes_lgl_eval %>% 
        select(loc_code, ser_length,
               ser_nryears,ser_firstyear,
               ser_lastyear) %>% 
        rename(watinacode = loc_code,
...
sel_cat1A_table <- sel_cat1A_raster %>% 
  ...
  summarise(gewenst_aantal_meetpunten = sum(gew_aantal_meetptn),
            aantal_peilbuizen =  sum(n_tubes, na.rm = TRUE)) %>% 
  arrange(rasterid)

kable(sel_cat1A_table %>%
        group_by(groupnr ) %>% 
        summarise('aantal cellen zonder peilbuis' = n(),
                  'aantal locaties zonder pb' =  sum(gewenst_aantal_meetpunten ) - sum(aantal_peilbuizen)) %>% 
        rename(GTgroep = groupnr
               ) ,
...
sel_cat1A_tm
```
Er zijn dus `r sum(sel_cat1A_table $gewenst_aantal_meetpunten) - sum(sel_cat1A_table $aantal_peilbuizen)` gewenste meetlocaties zonder peilbuis, verspreid over `r nrow(sel_cat1A_table)` rastercellen. 

### Categorie 1B: Rastercellen met een onvoldoend aantal geschikte peilbuizen


Categorie 1B is een selectie van deze tabel \@ref(tab:join-tubes-on-raster).
Het groepeert namelijk de rastercellen waarvoor het gewenst aantal meetpunten kleiner is dan het aantal peilbuizen met een goede tijdrijks.
Het resultaat van deze selectie wordt gegeven in tabel \@ref(tab:cat1B-table).
Voor deze groep kan onderzocht worden of door het modelmatig verbeteren van de tijdreeksen er wel aan de criteria kan voldaan worden.
...
#volgende berekening komt op hetzelfde neer, enkel rastercellen bestaande uit verschillende fragmenten worden dan als verschillende records gegeven
# sel_cat1B_raster <- sel_raster_meetnet %>% 
...
sel_cat1B_table <- sel_cat1B_raster %>% 
  st_drop_geometry() %>% 
  group_by(rasterid, groupnr) %>% 
  summarise(cel_teweinig_pb_met_lgl = n(),
            aantal_gew_meetptn_zonder_lgl = 
              sum(gew_aantal_meetptn) - sum(n_tubes_lgl)) %>% 
  arrange(rasterid)

kable(sel_cat1B_table %>% 
        rename(GTgroep = groupnr,
               'aantal cellen met onvoldoend aantal peilbuizen met een goede meetreeks' = cel_teweinig_pb_met_lgl,
                  'aantal gewenste meetlocaties zonder peilbuizen met een goede meetreeks' =  aantal_gew_meetptn_zonder_lgl
               ) ,
...
write_csv(tubes_group2, file.path(".", "data","local", "tubes_to_inspect.csv"))
...
Indien dan blijkt dat het aantal beschikbare meetpunten groter is dan het aantal dat gewenst is, wordt een keuze gemaakt dmv het 32m GRTS-raster. Het/de meetpunt(en) met de laagste rasternr(s) wordt/worden dan gekozen, in overeenstemming met het gewenst aantal meetpunten.
...
#functie om de meetpunten te clusteren waarvan hun laatste jaar met een kwalitatief goede tijdreeks niet meer dan zeker aantal jaar van elkaar verschilt 
