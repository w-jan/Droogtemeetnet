...
Indien een rastercel wel enkele peilbuizen heeft, weliswaar onvoldoende, dan zal deze rastercel tot meerdere categorieën behoren, afhankelijk van de kwaliteit van de meetreeksen. 
Als de kwaliteit van de tijdreeks van een aantal peilbuizen voldoende is om al onmiddellijk in een meetnet te kunnen worden opgenomen, wordt de cel ook tot categorie 2 gerekend. 
...
```
<br/><br/>
##Tijdreeksanalyse {#tijdreeksanalyse-uitvoering}
...
#wegfilteren van meetreeksen die niet in een raster valt (is mogelijk wanneer een peilbuis van een oude selectie in de resultaattabel van Menyanthes is verzeild)
tubes_menyanthes <- tubes_menyanthes %>% 
...
Tabel \@ref(tab:read-results-menyanthes) geeft een synthese van de tijdreeksanalyse. 
...
kable(tubes_cat1Ba %>% 
        ...
        arrange(watinacode),
      caption =  "Peilbuizen die op termijn tot het droogtemeetnet kunnen horen"
      ) %>% 
    kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"),
                full_width = T, 
                position = "left",
...
Doordat niet op alle meetreeksen een tijdreeksanalyse werd uitgevoerd, kan het zijn dat er nu weer rastercellen zijn waarbij meetreeksen van een oorspronkelijk lagere rang, nu wel de hoogste rang bekleden.
...
Tot nog toe kennen we enkel voor de cat. 1Ba en cat. 2 de exacte meetlocaties.
...
raster maken ter grootte van grid-grootte (8192m) waarmee de grote rasterbestanden kunnen verkleind (geknipt) worden
...
```{r excess-plot, eval=TRUE, echo=FALSE, message=TRUE, include=TRUE}
...
```{r excess-raster, eval=TRUE, echo=FALSE, message=TRUE, include=TRUE}
# sel_excess <- 
