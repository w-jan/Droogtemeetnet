...
We gaan hier uit van de ongelijke verdeling van de meetpunten over de strata (= GT-groepen), waarbij meer de drogere groepen relatief meer gewicht krijgen: voor de verdeling zi tabel \@ref(tab:GTgroepen-verdeling-over-strata).
...
Figuur \@ref(fig:selectie-rastercel-met-meetpunt-fig1) geeft toont de selectie voor GT-groep 1, de groep habitattypen van zeer natte bodems. Er werden 11 cellen geselecteerd die allemaal in het oosten gelegen zijn.
...
Ongeacht of er peilbuizen in deze rastercellen staan, zullen er in deze cellen nog meetpunten met een peilbuis moeten uitgerust worden.
...
Voor peilbuizen van rastercellen van categorie 2 betekent het niet weerhouden van de tijdreeks dat de tijdreeks van de peilbuis niet onmiddellijk kan gebruikt worden in het meetnet / de indicator.
...
Voor peilbuizen van rastercellen van categorie 3 
...
```{r cat2-plot-bis, fig.cap="rastercellen met een juist voldoende aantal peilbuizen", warning= FALSE, include=FALSE}
...
kobj <- kable(tubes_cat2_bis %>% 
        mutate(Inzetbaarheid = factor(if_else(selectie == -1, 
                                              "potentieel",
                                              "actueel"))) %>%    
        select(rasterid, groupnr, loc_code, Inzetbaarheid) %>% 
        rename(watinacode = loc_code,
               rastercelnummer = rasterid,
               GTgroep = groupnr
               ) ,
...
```{r cat3-plot-bis, fig.cap = "rastercellen met een overschot aan peilbuizen", warning= FALSE, include=FALSE}
...
```{r tubes-cat3-bis-table, message=FALSE}
...
kobj <- kable(tubes_cat3_bis %>% 
        ...
        select(loc_code, nryears, firstyear, lastyear, Inzetbaarheid) %>% 
        rename(watinacode = loc_code,
               'aantal jaren met lg3' = nryears,
               'eerste lg3-jaar' = firstyear,
               'recentste lg3-jaar' = lastyear
...
if (nognietklaar == TRUE) {
  export <- bind_rows(
    tubes_cat3_bis %>% mutate (cat = 3) %>% 
      select(loc_code, nryears, firstyear, lastyear, selectie, cat) , 
  ...
  kobj <- kable( bind_rows(
        tubes_cat3_bis %>% 
          select(loc_code, nryears, firstyear, lastyear, selectie) , 
        tubes_cat2_bis %>% 
          select(loc_code, nryears, firstyear, lastyear, selectie) , 
      ) %>% 
        filter(selectie == 0) %>% 
...
Figuur \@ref(fig:tubes-excess-plot) geeft geografisch weer tussen welke meetpunten gezocht kan worden.
Er is hierbij een verschil gemaakt tussen peilbuizen van actueel lage kwaliteit en van peilbuizen met modelleerbare tijdreeksen. 
...
kobj <- kable(tubes_selected %>%
      select(rasterid, groupnr, area_name, loc_code, x, y, selectie) %>%
...
Slechts `r tubes_current_GT %>% filter(groupnr == 2) %>% pull(act_inzetbaar)`% van de punten zijn actueel inzetbaar.
Dit wordt wel grotendeels gecompenseerd door de meetpunten van GT-groep 1. 
Als we beide GT-groepen samennemen, is het percentage `r tubes_current_GT %>% filter(group2 == 2) %>% slice(1) %>% pull(act_inzetbaar2)`%
