...
sel_cat1A_tm
```
Er zijn `r sum(sel_cat1A_table$aantal_cat1A)` gewenste meetlocaties zonder peilbuis, verspreid over `r nrow(sel_cat1A_table)` rastercellen. 
...
sel_cat1B_tm
```
Er zijn `r sum(sel_cat1B_table$aantal_cat1B)` gewenste meetlocaties met peilbuis echter zonder goede tijdreeks, verspreid over `r nrow(sel_cat1B_table)` rastercellen. 
De hiaten zijn bijgevolg ruimtelijk goed verdeeld.
...
```{r tubes-with-lg3-but-no-lgl, eval= TRUE, include= TRUE, echo= FALSE}
...
write_csv(tubes_group1B, file.path(".", "data","local", "tubes_to_inspect.csv"))
...
Zo kan er een groep cellen onderscheiden worden waarbij het gewenste aantal meetpunten juist overeenstemt met het aantal beschikbare peilbuizen met een lgl.
Hierbij wordt rekening gehouden dat er tussen de beschikbare peilbuizen nog kwaliteitsverschillen bestaan (op het vlak van het aantal meetjaren en de tijdsperiode van de metingen). 
Deze rastercellen behoren tot categorie 2.

De laatste groep, categorie 3, tenslotte omvat de rastercellen waarbij er meer peilbuizen met een lgl, gelijkwaardig van kwaliteit, bestaan dan het gewenst aantal meetpunten. Voor deze laatste groep zal nog een selectie van peilbuizen dienen te gebeuren.

#### Het bepalen van de bijkomende kwaliteitscriteria
In sectie \@ref(suppl-criteria) werden een aantal bijkomende kwaliteitscriteria opgegeven.
...
De grootte van deze clusters wordt dan vergeleken met het resterend ( = gewenst aantal - aantal van cat. 1A en cat. 1B) aantal gewenste meetlocaties voor een rastercel. 
Kan het resterend aantal ingevuld worden met de som van de x beste clusters, dan rekenen we die rastercel tot cat. 2. 
