...
write_vc(ruwedata01, file.path("data", "local", "ruwedata01"), sorting = c("dag","meetpunt_import"), strict= FALSE)
test <- read_vc(file.path("data", "local", "ruwdata01"))
start_time <- Sys.time()
test2 <- read_vc(file.path("data", "local", "testdata"))
end_time01 <- Sys.time()
...
schrikkeljaar <- testdata %>% 
  filter(meetpunt_import == 
           testdata %>% 
           filter(meetpunt_import != meetpunt) %>% 
           dplyr::select(meetpunt_import) %>% 
           head(1) %>% 
           dplyr::pull(meetpunt_import)
         ) %>% 
...
modelfitting <- function(indic_basis, respons, gemid, og, bg) {
  ...
  p <- p + theme(text = element_text(size = 15))
  p <- p + geom_point(shape = 16, size = 2, col = "black")
  p <- p + geom_line(aes_string(x = "jaar", y = gemid))
  p <- p + geom_ribbon(aes_string(x = "jaar",
                                  ymax = bg,
                                  ymin = og),
                       fill = grey(0.5),
                       alpha = 0.4)
...
modelkeuze <- data.frame(percentiel = c("01", "05", "10", "30" ), model = c("gaussian", "gamma", "gaussian", "gamma"))
...
indic_cum_p01_finaal <- indic_cum_p01_jaar_stat %>% 
  ...
  summarise(gem_jaar = mean(mean),
         ...
         sd_jaar = sd_jaar_biased + (20 + 1)/20 * 20/(20-1) * mean((gem_jaar - mean)^2),
...
#plot van de trend
gplot <- ggplot(data = indic_cum_p01_finaal, aes(x = jaar, y = gem_jaar)) + 
...
fig <- modelfitting(indic_basis = indic_cum_basis, respons = "lengte_onder_p01", gemid = "p01_mean_fitted", og = "p01_p02.5_fitted", bg = "p01_p97.5_fitted")
...
#plot van de trend
gplot <- ggplot(data = indic_cum_p05_finaal, aes(x = jaar, y = gem_jaar)) + 
  geom_line(aes(x = jaar, y = og_jaar), linetype = "longdash") +
  geom_ribbon(aes(x = jaar, ymax = bg_jaar, ymin = og_jaar),
              fill = grey(0.5),
              alpha = 0.4) +  
...
fig <- modelfitting(indic_basis = indic_cum_basis, respons = "lengte_onder_p05", gemid = "p05_mean_fitted", og = "p05_p02.5_fitted", bg = "p05_p97.5_fitted")
...
#plot van de trend
gplot <- ggplot(data = indic_cum_p10_finaal, aes(x = jaar, y = gem_jaar)) + 
  geom_line(aes(x = jaar, y = og_jaar), linetype = "longdash") +
  geom_ribbon(aes(x = jaar, ymax = bg_jaar, ymin = og_jaar),
              fill = grey(0.5),
              alpha = 0.4) +  
...
fig <- modelfitting(indic_basis = indic_cum_basis, respons = "lengte_onder_p10", gemid = "p10_mean_fitted", og = "p10_p02.5_fitted", bg = "p10_p97.5_fitted")
...
#plot van de trend
gplot <- ggplot(data = indic_cum_p30_finaal, aes(x = jaar, y = gem_jaar)) + 
  geom_line(aes(x = jaar, y = og_jaar), linetype = "longdash") +
  geom_ribbon(aes(x = jaar, ymax = bg_jaar, ymin = og_jaar),
              fill = grey(0.5),
              alpha = 0.4) +  
...
fig <- modelfitting(indic_basis = indic_cum_basis, respons = "lengte_onder_p30", gemid = "p30_mean_fitted", og = "p30_p02.5_fitted", bg = "p30_p97.5_fitted")
...
indic_abs_basis <- testdata %>% 
  ...
  group_by(meetpunt, simulatienr,jaar, aantaldagen_jaar) %>% 
...
checkdistributie <- indic_abs_basis %>% 
                                 filter(jaar == 2004, meetpunt == "ALMP003") %>% 
        dplyr::select(dag_onder_p10)
# nbinom <- fitdistr(checkdistributie, "Negative Binomial")
# qqp(checkdistributie, "nbinom", size = nbinom$estimate[[1]], mu = nbinom$estimate[[2]])
# poisson <- fitdistr(checkdistributie, "Poisson")
# qqp(checkdistributie, "pois", lambda = poisson$estimate)
...
modelkeuze <- data.frame(percentiel = c("01", "05", "10", "30" ), model = c("zeroinflatednbinomial2", "nbinomial", "nbinomial", "nbinomial"))
modelkeuze$model <- as.character(modelkeuze$model)

indic_abs_function <- function(modeldata, respons, percentile, indicatorname, standardised) {

  prec.prior <- list(prec = list(param = c(0.001, 0.001)))
  model <- as.formula(paste(respons, "~", "1 + f(jaar, model =", "'rw1', scale.model = FALSE,
                          hyper = prec.prior)+ f(meetpunt, model = 'iid', hyper = prec.prior)", sep = " "))   
    
  resultname_stat <- paste0("indic_abs_p", percentile,"_jaar_stat", if (standardised == TRUE) ("_std"))
  resultname_fitted <- paste0("indic_abs_p", percentile,"_fitted", if (standardised == TRUE) ("_std"))
  ...
  for (i in seq(from = 1, to = 20)) {
    ...
    # # Assess overdispersion (variantie / aantal vrijheidsgraden ~ 1)
     # #voor binomiaal
     # Pi   <- I2_binom$summary.fitted.values[,"mean"]
     # ExpY <- Pi * mdata$aantaldagen_jaar
     # VarY <- Pi * mdata$aantaldagen_jaar * (1 - Pi)
     # E1   <- (mdata %>%
     #               dplyr::pull(!!indicatorname) - ExpY) / sqrt(VarY)
     # N    <- nrow(mdata)
     # p <- nrow(I2_binom$summary.fixed)
     # Dispersion <- sum(E1^2) / (N - p)
     # Dispersion
    ...
    # #voor negatief binomiaal
    # Pi  <- I2_untr$summary.fitted.values[,"mean"]
    # theta <- I2_untr$summary.hyperpar[1,"mean"]
    # E1 <- (mdata %>% 
    #          dplyr::pull(!!indicatorname)  - Pi) / sqrt(Pi + Pi^2 / theta)
    # sum(E1^2) / (nrow(mdata) - 1)
    # 
    # Pi  <- I2_untr_kleineu$summary.fitted.values[,"mean"]
    # theta <- I2_untr_kleineu$summary.hyperpar[1,"mean"]
    # E1 <- (mdata %>% 
    #          dplyr::pull(!!indicatorname)  - Pi) / sqrt(Pi + Pi^2 / theta)
    # sum(E1^2) / (nrow(mdata) - 1)    
    # 
    # 
    # 
    # Pi  <- I2_prior$summary.fitted.values[,"mean"]
    # theta <- I2_prior$summary.hyperpar[1,"mean"]
    # E1 <- (mdata %>% 
    #          dplyr::pull(!!indicatorname)  - Pi) / sqrt(Pi + Pi^2 / theta)
    # sum(E1^2) / (nrow(mdata) - 1)    
    # 
    ...
    #bekijk de gefitte waarden van het model
    # Plot the fitted values
    # Fit1     <- I2c$summary.fitted.values[,"mean"]
    # Fit1.025 <- I2c$summary.fitted.values$"0.025quant"
    # Fit1.975 <- I2c$summary.fitted.values$"0.975quant"
    # 
...
fig <- modelfitting(indic_basis = indic_abs_basis, respons = "dag_onder_p01", gemid = "p01_mean_fitted", og = "p01_p02.5_fitted", bg = "p01_p97.5_fitted")
...
gplot <- ggplot(data = indic_abs_p05_finaal, aes(x = jaar, y = gem_jaar)) + 
  geom_line(aes(x = jaar, y = og_jaar), linetype = "longdash") +
  geom_ribbon(aes(x = jaar, ymax = bg_jaar, ymin = og_jaar), 
              fill = grey(0.5),
              alpha = 0.4) +  
...
fig <- modelfitting(indic_basis = indic_abs_basis, respons = "dag_onder_p05", gemid = "p05_mean_fitted", og = "p05_p02.5_fitted", bg = "p05_p97.5_fitted")
...
gplot <- ggplot(data = indic_abs_p10_finaal, aes(x = jaar, y = gem_jaar)) + 
  geom_line(aes(x = jaar, y = og_jaar), linetype = "longdash") +
  geom_ribbon(aes(x = jaar, ymax = bg_jaar, ymin = og_jaar), 
              fill = grey(0.5),
              alpha = 0.4) +  
  geom_line(aes(x = jaar, y = bg_jaar), linetype = "longdash") +
  geom_line(color = "dark blue") +
  # geom_point(data = indic_abs_basis, aes(x = jaar, y = duur_onder_p10)) +
  geom_hline(aes(yintercept = 0), linetype = "dotted") +
  labs(x = "Jaar", y = "trend")
...
fig <- modelfitting(indic_basis = indic_abs_basis, respons = "dag_onder_p10", gemid = "p10_mean_fitted", og = "p10_p02.5_fitted", bg = "p10_p97.5_fitted")
...
gplot <- ggplot(data = indic_abs_p30_finaal, aes(x = jaar, y = gem_jaar)) + 
  geom_line(aes(x = jaar, y = og_jaar), linetype = "longdash") +
  geom_ribbon(aes(x = jaar, ymax = bg_jaar, ymin = og_jaar), 
              fill = grey(0.5),
              alpha = 0.4) +  
  geom_line(aes(x = jaar, y = bg_jaar), linetype = "longdash") +
  geom_line(color = "dark blue") +
  # geom_point(data = indic_abs_basis, aes(x = jaar, y = duur_onder_p30)) +
  geom_hline(aes(yintercept = 0), linetype = "dotted") +
  labs(x = "Jaar", y = "trend")
...
fig <- modelfitting(indic_basis = indic_abs_basis, respons = "dag_onder_p30", gemid = "p30_mean_fitted", og = "p30_p02.5_fitted", bg = "p30_p97.5_fitted")
...
checkdistributie <- indic_rel %>% 
  filter(jaar == 2004, meetpunt == "ASBP003") %>% 
  dplyr::pull(rdag_onder_p10)
...
modelkeuze <- data.frame(percentiel = c("01", "05", "10", "30" ), model = c("zeroinflatednbinomial0", "zeroinflatednbinomial1", "nbinomial", "gaussian"))
modelkeuze$model <- as.character(modelkeuze$model)
...
indic_rel_function <- function(modeldata, respons, percentile, indicatorname, standardised) {
  ...
  for (i in seq(from = 1, to = 20)) {
    ...
    # 
    # Fit1     <- I3b$summary.fitted.values[,"mean"]
    # Fit1.025 <- I3b$summary.fitted.values$"0.025quant"
    # Fit1.975 <- I3b$summary.fitted.values$"0.975quant"
    # 
    # #
...
fig <- modelfitting(indic_basis = indic_rel, respons = "rdag_onder_p01", gemid = "p01_mean_fitted", og = "p01_p02.5_fitted", bg = "p01_p97.5_fitted")
...
gplot <- ggplot(data = indic_rel_p05_finaal, aes(x = jaar, y = gem_jaar)) + 
  geom_line(aes(x = jaar, y = og_jaar), linetype = "longdash") +
  geom_ribbon(aes(x = jaar, ymax = bg_jaar, ymin = og_jaar), 
              fill = grey(0.5),
              alpha = 0.4) +  
  geom_line(aes(x = jaar, y = bg_jaar), linetype = "longdash") +
  geom_line(color = "dark blue") +
  # geom_point(data = indic_rel, aes(x = jaar, y = rdag_onder_p05)) +
  geom_hline(aes(yintercept = 0), linetype = "dotted") +
  labs(x = "Jaar", y = "trend")
...
fig <- modelfitting(indic_basis = indic_rel, respons = "rdag_onder_p05", gemid = "p05_mean_fitted", og = "p05_p02.5_fitted", bg = "p05_p97.5_fitted")
...
gplot <- ggplot(data = indic_rel_p10_finaal, aes(x = jaar, y = gem_jaar)) + 
  geom_line(aes(x = jaar, y = og_jaar), linetype = "longdash") +
  geom_ribbon(aes(x = jaar, ymax = bg_jaar, ymin = og_jaar), 
              fill = grey(0.5),
              alpha = 0.4) +  
  geom_line(aes(x = jaar, y = bg_jaar), linetype = "longdash") +
  geom_line(color = "dark blue") +
  # geom_point(data = indic_rel, aes(x = jaar, y = duur_onder_p10)) +
  geom_hline(aes(yintercept = 0), linetype = "dotted") +
  labs(x = "Jaar", y = "trend")
...
fig <- modelfitting(indic_basis = indic_rel, respons = "rdag_onder_p10", gemid = "p10_mean_fitted", og = "p10_p02.5_fitted", bg = "p10_p97.5_fitted")
...
gplot <- ggplot(data = indic_rel_p30_finaal, aes(x = jaar, y = gem_jaar)) + 
  geom_line(aes(x = jaar, y = og_jaar), linetype = "longdash") +
  geom_ribbon(aes(x = jaar, ymax = bg_jaar, ymin = og_jaar), 
              fill = grey(0.5),
              alpha = 0.4) +  
  geom_line(aes(x = jaar, y = bg_jaar), linetype = "longdash") +
  geom_line(color = "dark blue") +
  # geom_point(data = indic_rel, aes(x = jaar, y = duur_onder_p30)) +
  geom_hline(aes(yintercept = 0), linetype = "dotted") +
  labs(x = "Jaar", y = "trend")
...
fig <- modelfitting(indic_basis = indic_rel, respons = "rdag_onder_p30", gemid = "p30_mean_fitted", og = "p30_p02.5_fitted", bg = "p30_p97.5_fitted")
