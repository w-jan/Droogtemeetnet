...
rasterid_grid <- sel_raster_group1_df[2,1] %>% as.integer()
...
# habmap_raster_gwgroup <- raster::rasterize(habmap_polygons_gw %>% 
#                                              filter(groupnr == gwgroup),
#                                               clip1,
#                                               field = "groupnr", 
#                                               mask = FALSE)
habmap_raster_unieknr <- raster::rasterize(habmap_polygons_gw %>%
                                             filter(groupnr == gwgroup & selecteerbaar == 1),
                                              clip1,
                                              field = "unieknr",
                                              mask = FALSE)
habmap_raster_rangnr <- raster::rasterize(habmap_polygons_gw %>% 
                                             filter(groupnr == gwgroup & selecteerbaar == 1),
                                              clip1, 
                                              mask = TRUE)
...
grid_rangnr <- grid_rangnr %>% 
  ...
  distinct() %>% 
  arrange(celwaarde) 
grid_rangnr_min <- grid_rangnr %>% slice(1) %>% as.integer()   

clip1_min <- clip1[clip1 == grid_rangnr_min, drop = FALSE]
clip6_1cel <- grts_level6[clip1_min, drop = FALSE]
# raster::plot(clip1_min)
# raster::plot(clip6_1cel, add = FALSE)

clip6 <- grts_level6[grts_level6 == raster::getValues(clip6_1cel), drop = FALSE]
# plot(clip1_min) 
...
habmap_polygons_gw <- 
  habmap_polygons_gw %>% 
    left_join(habmap_polygons_gw_part, 
              by = "unieknr") %>% 
    mutate(selecteerbaar = ifelse(is.na(geselecteerd), selecteerbaar, 0)) %>% 
    select(-geselecteerd)

#opzoeken reservepunten
#habitatkaart verrasteren binnen deze gridcel

#ophalen van de unieke nummers
habmap_raster_unieknr6 <- habmap_raster_unieknr[clip6, drop = TRUE] %>% 
  as.data.frame()
names(habmap_raster_unieknr6) <- "unieknr" 
habmap_raster_unieknr6 <- habmap_raster_unieknr6 %>% 
  filter(!is.na(unieknr)) %>%  
  distinct() %>% 
  arrange(unieknr) %>% 
  mutate(geselecteerd = 1)
...
habmap_polygons_gw <- 
  habmap_polygons_gw %>% 
    left_join(habmap_raster_unieknr6, 
              by = "unieknr") %>% 
