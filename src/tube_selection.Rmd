...
tubes_to_inspect <- tubes_lg3_avail %>% 
  inner_join(tubes_in_raster %>% 
               select(starts_with("loc"), rasterid, groupnr) %>% 
               distinct, by = "loc_code") %>% 
  inner_join(sel_points_no_lgl %>% 
               st_drop_geometry(), by = c("rasterid", "groupnr")) %>%
  filter(nryears >= 0) %>% 
  arrange(rasterid, groupnr, desc(nryears), desc(firstyear)) %>% 
  select(rasterid, groupnr, gew_aantal_meetptn, loc_code, everything(), -xg3_variable, -starts_with("loc_t"), -starts_with("loc_v"))
