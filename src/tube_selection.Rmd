...
#save it as a git2rdata-object
tubes_hab <- tubes_hab %>% 
  arrange(loc_code, polygon_id, type)
write_vc(tubes_hab, file.path("..","data","tubes_hab"), sorting = c("loc_code", "polygon_id", "patch_id", "type" ), strict =  FALSE)

test <- tubes_hab %>% 
  count(loc_code, polygon_id, type)
...
tubes_hab <- tubes_hab %>% 
  semi_join(tubes_hab_groep, by = c("loc_code", "polygon_id", "groupnr" ))
...
sel_no_tube_table <- sel_raster_meetnet %>% 
  anti_join(tubes_in_raster, group_by = c("rasterid", "groupnr")) %>% 
  st_drop_geometry() %>% 
  distinct(rasterid, groupnr) %>% 
  arrange(rasterid)
...
tubes_to_inspect <- tubes_lg3_avail %>% 
  inner_join(tubes_in_raster %>% 
               select(loc_code, rasterid, groupnr) %>% 
               distinct, by = "loc_code") %>% 
...
write_csv(tubes_to_inspect, paste0("../data/", "tubes_to_inspect.csv"))
...
```{r using GRTS for refining tubeselection}
sel_toomuch_lgl_vb <- tubes_in_raster %>% 
  inner_join(sel_raster_meetnet %>% 
               select(rasterid, groupnr, gew_aantal_meetptn), by =  c("rasterid", "groupnr")) %>% 
...
# check <- sel_toomuch_lgl_vb %>% 
#   filter(rasterid == 134, groupnr == 4)

# sel_toomuch_lgl_vb <- sel_toomuch_lgl_vb %>%  
#   mutate(gew_aantal_meetptn = case_when(
#     rasterid == 134 & groupnr == 4 ~ 6,
...
einde <- 0
  for (i in seq(1:aantal_jaren)) {
    # i <- 2
    check_rank <- x %>% 
      filter(minrank == i)
    beschikbaar_aantal_rank <- x[x$minrank == i, "n"] %>% as.integer()
...
sel_toomuch_lgl_maxrank <- plyr::ddply(sel_toomuch_lgl_vb, ~rasterid+groupnr, max_rank) %>%
  rename(maxrank = V1)

sel_toomuch_lgl <- sel_toomuch_lgl_vb %>% 
  left_join(sel_toomuch_lgl_maxrank, by = c("rasterid", "groupnr")) %>% 
  filter(minrank <= maxrank) %>% 
  group_by(rasterid, groupnr, gew_aantal_meetptn, maxrank) %>% 
  summarise(beschikbaar_aantal = sum(n)) %>% 
  ungroup %>% 
  inner_join(sel_toomuch_lgl_vb %>%
...
#bijhorende meetpunten ervan opzoeken
tubes_excess <- tubes_in_raster %>% 
  distinct(loc_code, x, y, rasterid, groupnr) %>% 
  inner_join(tubes_lg3_eval, by =  "loc_code") %>% 
  inner_join(sel_toomuch_lgl, by =  c("rasterid", "groupnr", "ser_lastyear"))
