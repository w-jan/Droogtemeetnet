...
De absolute indicatorwaarde is het aantal dagen op een jaar $j$ dat het grondwaterpeil op een meetlocatie $m$ onder een kritische drempelwaarde zit (cfr. \@ref(h3:indicator)). 

```

We berekenen deze waarde voor elke simulatie $s$ afzonderlijk en geven deze de notatie $d_{mjs}$.
Voor de periode 1985 - 2018, 58 meetlocaties en 20 simulaties betekent dit dus 34 * 58 * 20 = 39440 $d_{mjs}$-waarden
...
Er zijn echter een honderdtal locaties voorzien.
...
Kennen we de variantie kan hiermee, op basis van de centrale limietstelling, voor elke $DI_j$ een betrouwbaarheidsinterval berekend worden.
...
### lopend gemiddelde {#h3:calc-rollend-gemiddelde}
...
Men heeft daarenboven nog verschillende mogelijkheden om een lopend gemiddelde te berekenen, ongewogen of bijv. gewogen op basis van een lineaire trend, exponentiële functie etc.
...
Bijv. drie opeenvolgende droge jaren kunnen een zwaardere impact hebben dan wanneer deze droge jaren onderbroken werden met een nat jaar. 
...
We berekenen een lopend gemiddelde voor een aantal tijdvensters (2, 4, 6, 8, 10, 20 en 30 jaar). 
Net zoals bij de originele waarden kan ook voor een lopend gemiddelde met behulp van de formule van Rubin  een betrouwbaarheidsinterval bepaald worden. 
...
Daarenboven wordt ook aangenomen dat een meetpunt een vaste (maar onbekende) bijdrage aan de droogte-indicator levert (dus dat een meetpunt systematisch droger of natter dan een andere kan zijn).
...
Het model laat niet toe mogelijke lokale/regionale verschillen te detecteren. 
...
Hier kan wel bij opgemerkt worden dat 1) bij de selectieprocedure van de meetpunten, meetpunten geweerd werden die tijdens de referentieperiode een significante trend (verdroging/vernatting) vertonen, die niet door klimatologische factoren kan verklaard worden en 2) voor elk meetpunt afzonderlijk een specifieke kritische drempelwaarde werd bepaald.

Om de eerste opmerking zullen, vooral voor de referentieperiode, er over de jaren heen tussen de punten weinig grote verschuivingen optreden. 
...
Het is deze term en meer nog de veralgemening voor alle simulaties die ons hier interesseert.
