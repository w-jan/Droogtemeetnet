...
```{r nat-indicator, include=FALSE}
nat_ind_j <- dagen_droog %>% 
  group_by(jaar) %>% 
...
```{r nat-indicator-met-ci, include=FALSE}

nat_ind_j_s <- dagen_droog %>% 
  group_by(jaar, simulatienr) %>% 
...
```{r nat-indicator-met-ci-vc, include=FALSE}
write_vc(nat_ind_j, file.path("data", "result", "nat_indicator"), sorting = c("jaar"), root = projroot)
```
...
g <- ggplot(data = nat_ind_j, aes(x = jaar, y = nat_ind_j)) + 
  theme_inbo() +
  geom_ribbon(aes(ymin = lcl, ymax = ucl), fill = inbo_lichtgrijs) +
  geom_hline(yintercept = 365 * 0.05, color = inbo_oranje, linetype = "dashed") +
  geom_line(color = inbo_rood, size = 0.5) + 
...
indic_abs_function <- function(modeldata, suffix= "", respons, percentile, indicatorname, standardised, cache, tijdvenster) {
  ...
  saveRDS(dagen_droog, file.path(cache, paste0("modelresultaat_", respons, ".rds" )))
