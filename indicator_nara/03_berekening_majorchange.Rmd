...
```{r abs-indicator, eval = !file.exists(file.path("cache", "dagendroognat.rds" ))}

dagen_droog_nat <- ruwedata %>% #oude objectnaam: indic_abs_basis
...
```
```{r rollend-gemiddelde, eval = !file.exists(file.path("cache", "dagendroognat.rds" ))}
...
#wegschrijven in een RDS-bestand
saveRDS(dagen_droog_nat,file.path("cache", "dagendroognat.rds" ))
...
workflow <- "nietrekenen"
```

```{r abs-indic-model-without-trend-droog, eval = !file.exists(file.path("cache", paste0("modelresultaat_geentrend.rds")))}
...
dagen_droog_nat <- indic_abs_function_without_trend(modeldata = modeldata, respons = respons, percentile = percentile, standardised = standardised, cache = "cache" )
...
```{r abs-indic-model-without-trend-nat, eval = !file.exists(file.path("cache", paste0("modelresultaat_geentrend.rds"))) | workflow == "rekenen"}
dagen_droog_nat <- indic_abs_function_without_trend(modeldata = modeldata, respons = respons, percentile = percentile, standardised = standardised, cache = "cache" )
```


```{r abs-indic-model-without-trend-uit-cache, include=FALSE}
dagen_droog_nat <- readRDS(file.path("cache", paste0("modelresultaat_geentrend.rds" ) ))
...
```{r abs-indic-model, include=FALSE, eval = !file.exists(file.path("cache", paste0("modelresultaat_", respons, ".rds" )))}
#bij een eventuele herberekening moet de velden met de rekenresultaten eerst verwijderd worden
dagen_droog_nat <- dagen_droog_nat %>% 
  dplyr::select(-contains("fitted") )
...
dagen_droog_nat <- indic_abs_function(modeldata = modeldata, respons = respons, percentile = percentile, indicatorname = indicatorname, standardised = standardised, cache = "cache", tijdvenster = 0 )

```

```{r abs-indic-model-droog-uit-cache, include=FALSE}
dagen_droog_update <- readRDS(file.path("cache", paste0("modelresultaat_", respons, ".rds" ) ))
for (ma in tijdvenster) {
...
```{r abs-indic-model-nat, include=FALSE, eval = !file.exists(file.path("cache", paste0("modelresultaat_", respons, ".rds" )))}
#bij een eventuele herberekening moet de velden met de rekenresultaten eerst verwijderd worden

modelkeuze <- data.frame(percentiel = c("01", "05", "10", "30", "95" ), model = c("zeroinflatednbinomial2", "zeroinflatednbinomial0", "zeroinflatednbinomial0", "nbinomial","zeroinflatednbinomial0"))
modelkeuze$model <- as.character(modelkeuze$model)

dagen_droog_nat <- indic_abs_function(modeldata = modeldata, respons = respons, percentile = percentile, indicatorname = indicatorname, standardised = standardised, cache = "cache", tijdvenster = tijdvenster)
...
#bewaren resultaat
write_vc(dagen_droog_update, file.path("data", "result", "dagen_droog_nat"), sorting = c("jaar", "meetpunt", "simulatienr"), strict = FALSE)
...
#berekenen van indicator
indic_abs_nat_gem <- dagen_droog_update %>% 
  group_by(jaar) %>% 
...
De verticale rode lijnen geeft het bereik van het geschatte verwachte aantal droge dagen per jaar voor de 58 locaties. 
...
#tabel met de indicatorwaarden (rollend gemiddelden) en trend berekening (model) omvormen naar een lange versie
dagen_nat_lang_breed <- dagen_droog_update %>% 
...
#bewaren resultaten
write_vc(droogte_trend_05, file.path("data", "result", "droogte_trend_05"), sorting = c("jaar", "tijdvenster"), root = projroot)

dagen_droog_lang_breed <- dagen_droog_lang_breed %>% 
  dplyr::select(-contains(c("boven", "p95")))
write_vc(dagen_droog_lang_breed, file.path("data", "result", "droogte_fitting_05"), sorting = c("meetpunt", "jaar", "simulatienr", "tijdvenster"), root = projroot)
```

```{r trend-nat-vc, include=FALSE}
#bewaren resultaten
write_vc(nat_trend_95, file.path("data", "result", "nat_trend_05"), sorting = c("jaar", "tijdvenster"), root = projroot)

dagen_nat_lang_breed <- dagen_nat_lang_breed %>% 
  dplyr::select(-contains(c("onder", "p05")))
write_vc(dagen_nat_lang_breed, file.path("data", "result", "nat_fitting_95"), sorting = c("meetpunt", "jaar", "simulatienr", "tijdvenster"), root = projroot)
```
...
write_csv(droogte_trend_05 %>% 
            ...
            rename(`aantal dagen onder kritisch minimum` = droogte_ind_j,
                   ...
                   bg_jaar_indicator = ucl), file.path("data", "result", "figuur_droog_nara.csv"))
```
...
write_csv(nat_trend_95 %>% 
            ...
            rename(`aantal dagen boven hoog drempelpeil` = nat_ind_j,
                   ...
                   bg_jaar_trend = bg_jaar,
                   og_jaar_indicator = lcl,
                   bg_jaar_indicator = ucl), file.path("data", "result", "figuur_nat_nara.csv"))
```
