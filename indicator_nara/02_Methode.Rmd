...
Eerst tellen we voor elke meetlocatie, elk jaar en elke simulatie het aantal dagen dat het grondwaterpeil onder de kritische drempelwaarde is = ${\text {dagen_droog}}_{meetlocatie_m; simulatie_s; jaar_j}$. Voor de periode 1985 - 2018, 58 meetlokaties en 20 simulaties betekent dit dus 34 * 58 * 20 = 39440 ${\text {dagen_droog}}$-waarden

```{block2 definitie-droogte-indicator, type="exampleblock", latex.options="{}", echo = TRUE}
De droogte-indicator voor een bepaald jaar $j$ is dan het gemiddelde van deze aantallen droge dagen voor alle simulaties en meetlocaties: ${\text {droogte-ind}}_{j} = \frac{\sum_{s=1}^{20}\sum_{m=1}^{58}{\text {dagen_droog}}_{m;s;j}}{20*58}$
...
$${\text {de droogte-indicator voor één simulatie en één jaar : droogte-ind-sim}}_{j;s} =  \sum_{m = 1}^{58}({\text {dagen_droog}}_{m;s;j}) / 58 $$
$${\text {de droogte-indicator voor één jaar : droogte-ind}}_{j} =  \sum_{s = 1}^{20}(\frac{\sum_{m=1}^{58}{\text {dagen_droog}}_{m;s;j}}{58}) / 20 $$
...
$$ {\text {LCL}}_j = {\text{droogte-ind}}_{j} - 1.96 * \sqrt{\bar\sigma^2_{{\text {droogte-ind}}_{j}}}$$

De bovengrens van het betrouwbaarheidsinterval (UCL) voor een bepaald jaar $j$ wordt dan :

$$ {\text {UCL}}_j = {\text{droogte-ind}}_{j} + 1.96 * \sqrt{\bar\sigma^2_{{\text {droogte-ind}}_{j}}}$$
...
$${\text {dagen_droog}}_{m;s;j} \sim NB(\overline{{\text {droogte-ind-sim}}}_{j;s}, \theta)  $$

$$\overline{{\text {droogte-ind-sim}}}_{j;s} = E({\text {dagen_droog}}_{m;s;j}) \sim {\text {jaar}} + (1 \vert{\text{meetlocatie}})  $$
of 

$$\log(\overline{{\text {droogte-ind-sim}}}_{j;s}) = \beta_0 + \beta_1T_j + t_j + b_m + \epsilon_{jm}$$
...
$${\text {de trend van de droogte-indicator berekend voor één jaar : }} \overline{{\text {droogte-ind}}}_{j} =  \sum_{s = 1}^{20}(\overline{{\text {droogte-ind-sim}}}_{j;s} / 20 $$
...
$$ {\text {LCL}}_j = \overline{{\text {droogte-ind}}}_{j} - 1.96 * \sqrt{\bar\sigma^2_{\overline{{\text {droogte-ind}}}_{j}}}$$

De bovengrens van het betrouwbaarheidsinterval (UCL) voor een bepaald jaar $j$ wordt dan :

$$ {\text {UCL}}_j = \overline{{\text {droogte-ind}}}_{j} + 1.96 * \sqrt{\bar\sigma^2_{\overline{{\text {droogte-ind}}}_{j}}}$$
