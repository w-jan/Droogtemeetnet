...
```{r assigning-tubes-grts, cache=cache_assigning_tubes_grts, eval = !file.exists(file.path("cache", "tubes_cat3_grts.rds" ))}
#gecached !
...
saveRDS(tubes_cat3_grts, file.path("cache", "tubes_cat3_grts.rds" ))
saveRDS(tubes_cat3_grts_reserve, file.path("cache", "tubes_cat3_grts_reserve.rds" ))
```
...
tubes_cat3_grts <- readRDS(file.path("cache", "tubes_cat3_grts.rds" ))
tubes_cat3_grts_reserve <- readRDS(file.path("cache", "tubes_cat3_grts_reserve.rds" ))
...
```{r selection-polygon-cat1A, cache=cache_selection_polygon_cat1A, message=FALSE, warning= FALSE, eval = !file.exists(file.path("cache", "tubes_cat1_polyg.rds" ))}

 #wat voorbereidende databewerkingen aan de overlay habitatkaart en het raster (grid8)

 habmap_gw_raster_in_beheer <- habmap_gw_raster_in_beheer %>%
 ...
 for (i in seq(1:nrow(sel_cat1A_raster_df))) {
   ...
   for (j in seq(1:nrow(sel_cat1A_raster_1grid))) {
     ...
     habmap_raster_rangnr <- raster::rasterize(habmap_gw_raster_in_beheer %>% 
     ...
     habmap_raster_unieknr <- raster::rasterize(habmap_gw_raster_in_beheer %>%
                                                  filter(groupnr == gwgroup & selecteerbaar == 1),
                                                clip0,
                                                field = "unieknr",
                                                mask = FALSE)
...
saveRDS(tubes_cat1_polyg, file.path("cache", "tubes_cat1_polyg.rds" ))
saveRDS(habmap_gw_raster_in_beheer, file.path("cache", "habmap_gw_raster_in_beheer.rds" ))

```
```{r tubes-cat1-polyg-result}
tubes_cat1_polyg <- readRDS(file.path("cache", "tubes_cat1_polyg.rds" ))
habmap_gw_raster_in_beheer <- readRDS(file.path("cache", "habmap_gw_raster_in_beheer.rds" ))
```
...
tubes_selected_types <- 
  ...
  #as_tibble %>% 
  # filter(selectie == 1) %>% 
  dplyr::select(loc_code, x, y, selectie, 
...
write_sf(tubes_selected_sf %>%  
           dplyr::select(-obswell_statecode, -loc_validitycode) %>% 
...
tubes_current <- tubes_selected %>% 
  as_tibble() %>% 
  dplyr::select(-n) %>% #n uitsluiten, want de count-functie loopt hierdoor mis
  filter(selectie == 1) %>% 
  count(groupnr) 
