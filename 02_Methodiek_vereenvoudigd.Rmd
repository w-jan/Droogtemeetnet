...
[^1]: GRTS = Generalized Random Tessellation Stratified
...
In de praktijk wordt deze voorwaarde opgelegd door enkel die habitatvlekken en/of peilbuizen te selecteren die voor 50% of meer toegewezen kunnen worden aan eenzelfde GT-groep en die op minstens `r bufferpb` m afstand liggen van een andere GT-groep.
...
Deze opdeling is een hulp bij het ontwerp, omdat elke categorie een specifieke behandeling toelaat.
...
Vergelijken we dan de vraag, het gewenste aantal meetpunten per rastercel (= V), met het aanbod van meetpunten met een peilbuis (of kortweg het aantal peilbuizen = A) dan kunnen de rastercellen in drie categorieën verdeeld worden. 
De categorieën sluiten elkaar mutueel uit.

- Cat. 1: de vraag is groter dan het aanbod (V > A) 

Er zijn geen meetpunten met een peilbuis of er is een gebrek aan.

- Cat. 2: de vraag is gelijk aan het aanbod (V = A) 

Het aantal beschikbare meetpunten met een peilbuis stemt juist overeen met het aantal dat gezocht wordt.

- Cat. 3: de vraag is kleiner dan het aanbod (V < A) 
...
Het zal bijgevolg op basis van de tijdreeksanalyse zijn dat meetpunten als geschikt (er is tijdsmodel mogelijk) of niet geschikt (een model is niet mogelijk) zullen bevonden worden.
...
(@) Meetpunten met een _recente_ kwalitatief goede tijdreeks[^3] primeren boven meetpunten met een oudere tijdreeks. 
...
[^3]: Een kwalitatief goed meetjaar wordt hier gedefinieerd als een hydrologisch jaar waarvoor een lg3 kan berekend worden. We beschouwen enkel tijdreeksen vanaf 2000. Dit houdt in dat er minstens 20 goed gespreide metingen per jaar zijn. Het verschil tussen twee metingen mag niet groter zijn dan 30 dagen en de eerste en laatste meting moet resp. in april en maart (van het daaropvolgende kalenderjaar) vallen. 
...
```{r voorbeeld-selectie-part1, fig.cap="Voorbeeld van een gevolgde workflow bij de selectie", , fig.width=8, fig.height=8}
  include_graphics(path = file.path(".","figures", 
                                    "voorbeeld_selectie_part1.png")) 
```
```{r voorbeeld-selectie-part2, fig.cap="Voorbeeld van een gevolgde workflow bij de selectie (vervolg)", fig.width=8, fig.height=8}
  include_graphics(path = file.path(".","figures", 
